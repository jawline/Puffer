...
void assemble_udp_header(const ip *ip, udphdr *udp, size_t datagram_contents_size,
                         const sockaddr_in &sender, const sockaddr_in &destination) {
...
uint16_t checksum(const ip *ip, uint8_t *header, size_t header_len, uint8_t *data, size_t data_len,
                  uint8_t proto, uint32_t len) {
    return wrapsum(
            checksum((unsigned char *) header, header_len, checksum(data, data_len, checksum(
                    (unsigned char *) &ip->saddr, 2 * sizeof(ip->saddr), proto + len))));
}
            ...
            wrapsum(checksum((unsigned char *) tcp, sizeof(*tcp),
                             checksum((unsigned char *) (tcp + 1), datagram_contents_size,
                                      checksum((unsigned char *) &ip->saddr, 2 * sizeof(ip->saddr),
                                               ip->proto +
                                               (uint32_t) (ntohs(ip->len) - sizeof(*ip))))));

    tcp->check = sum;
}

ssize_t assemble_udp_packet(char *out, size_t mtu, char *data, size_t len, const sockaddr_in &src,
                            const sockaddr_in &dst) {
...
ssize_t
assemble_tcp_packet(char *out, size_t mtu, uint32_t seq, uint32_t ack, char *data, size_t len,
