...
private:
    ...
    inline void report(size_t udp, size_t tcp, size_t expired) const {
        ...
        debug("STATE: UDP: %zu / %zu (%zu / %zu) bytes TCP: %zu / %zu (%zu / %zu) "
              ...
              udp, stat.udp_total, stat.udp_bytes_in, stat.udp_bytes_out, tcp, stat.tcp_total,
        ...
        jni_env->CallVoidMethod(jni_service, report_method, (jlong) tcp, (jlong) stat.tcp_total,
                                ...
                                (jlong) (stat.tcp_bytes_out + stat.udp_bytes_out),
                                (jlong) stat.blocked);
    ...
    inline void
    process_packet_udp(struct ip *hdr, char *bytes, size_t len, timespec const &cur_time) {
        DROP_GUARD(len >= sizeof(struct udphdr));
        struct udphdr *udp_hdr = (struct udphdr *) bytes;
        struct ip_port_protocol id = ip_port_protocol{hdr->daddr, udp_hdr->uh_sport,
                                                      udp_hdr->uh_dport, IPPROTO_UDP};
        ...
        if (fd_scan != outbound_nat.end()) {
            ...
            udp_socket =
                    std::shared_ptr<Socket>(
                            new UdpStream(new_fd, generate_addr(hdr->saddr, udp_hdr->uh_sport),
                                          generate_addr(hdr->daddr, udp_hdr->uh_dport),
                                          IPPROTO_UDP));
        ...
        if (udp_socket->on_tun(tunnel_fd, epoll_fd, (char *) hdr, (char *) udp_hdr, bytes, len,
                               block, stat, cur_time)) {
    ...
    inline void
    process_packet_tcp(struct ip *hdr, char *bytes, size_t len, timespec const &cur_time) {
        ;

        DROP_GUARD(len >= sizeof(struct tcphdr));
        struct tcphdr *tcp_hdr = (struct tcphdr *) bytes;
        struct ip_port_protocol id = ip_port_protocol{hdr->daddr, tcp_hdr->source, tcp_hdr->dest,
                                                      IPPROTO_TCP};
        ...
        if (fd_scan != outbound_nat.end()) {
            ...
            if (fd_scan->second->on_tun(tunnel_fd, epoll_fd, (char *) hdr, (char *) tcp_hdr,
                                        data_start, data_size, block, stat, cur_time)) {
            ...
            if (!(tcp_hdr->syn && !tcp_hdr->ack)) {
                ...
                TcpStream::return_tcp_rst(tunnel_fd, generate_addr(hdr->saddr, tcp_hdr->source),
    ...
    inline void setup_jni_env() {
        ...
        report_conn_method = jni_env->GetMethodID(jni_service_class, "reportConn",
