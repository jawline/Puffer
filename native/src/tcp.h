...
public:
    ...
    TcpPart(char *src_data, uint32_t seq_start, size_t len, timespec const &cur_time) : seq_start(
            seq_start), seq_len(len), packet_time(cur_time) {
        fatal_guard(len <= MTU);
        memcpy(data, src_data, len);
    }

    bool acknowledged(uint32_t ack_nr) const { return ack_nr > seq_start + seq_len; }
...
private:
    ...
    inline bool
    retransmit_packet(int tun_fd, char *data, uint32_t us_seq_nr, size_t len, bool syn, bool ack,
                      bool fin) const {
        ...
        size_t pkt_size = assemble_tcp_packet(packet_buffer, MTU, us_seq_nr, them_seq + 1, data,
...
public:
    TcpStream(int fd, sockaddr_in src, sockaddr_in dst, uint8_t proto, uint32_t them_seq_start)
    ...
    inline bool should_block(char *data, size_t data_size, BlockList const &block) {
    ...
    bool on_tun(int tun_fd, int epoll_fd, char *ip, char *proto, char *data, size_t data_size,
                BlockList const &block, struct stats &stats,
                timespec const &cur_time) {
        ...
        if (is_out_of_order) {
            log("TCP %i: DATA: Out of order %u %u %u %u %zu", fd, tcp_seq, them_seq, us_seq,
                them_ack, unacknowledged.size());
        }
        ...
        if (data_size > 0 && !is_out_of_order) {
            ...
            debug("TCP %i: DATA: Sequence: %u (New Expected: %u) Size: %zu", fd, tcp_seq, them_seq,
                  data_size);
        ...
        if (tcp_hdr->fin) {
            them_seq += 1;
            shutdown(fd, SHUT_WR);
            debug("TCP %i: Client has shutdown write half of stream %u %u %i", fd, us_seq, them_ack,
                  close_rd);
            close_wr = true;
        ...
        if (tcp_hdr->ack && close_rd && !ack_rd && them_ack >= us_seq) {
            debug("TCP %i: Stream has acknowledged FIN %u %u %u with %zu remaining", fd, tcp_seq,
                  them_ack, us_seq, unacknowledged.size());
            ack_rd = true;
        }
        ...
        if (unacknowledged.size()) {
            ...
            for (auto segment : unacknowledged) {
                if (segment.should_retransmit(cur_time)) {
                    retransmit_packet(tun_fd, segment.data, segment.seq_start, segment.seq_len,
                                      false, true, close_rd);
                    done++;
                }
            }
    ...
    bool
    on_sock(int tun_fd, int epoll_fd, int events, struct stats &stats, timespec const &cur_time) {

        if (events & EPOLLOUT) {
            debug("TCP Connected - it's TIME TO SEND A SYN-ACK");
            debug("SYN-ACK numbers %u %u", us_seq, them_seq + 1);

            DROP_GUARD_RET(return_packet(tun_fd, nullptr, 0, true, true, false, cur_time), true);
            us_seq++;
        ...
        if (events & (EPOLLHUP | EPOLLRDHUP)) {
            debug("TCP %i: Upstream closed. Generating FIN with SYN-ACK numbers %u %u", fd, us_seq,
                  them_seq + 1);
    ...
    static inline void return_tcp_rst(int tun_fd, const sockaddr_in &src, const sockaddr_in &dst) {
        char ipp[MTU];
        size_t pkt_sz = assemble_tcp_packet(ipp, MTU, 0, 0, nullptr, 0, dst, src, false, false,
                                            false, false, true);
        DROP_GUARD(pkt_sz > 0);
