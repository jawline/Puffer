...
private:
  ...
  inline bool retransmit_packet(int tun_fd, char *data, uint32_t us_seq_nr, size_t len, bool syn, bool ack, bool fin) const {
    ...
    size_t pkt_size = assemble_tcp_packet(packet_buffer, MTU, us_seq_nr, them_seq + 1, data, len, dst, src, false, syn, ack, fin, false);
...
public:
  ...
  bool on_tun(int tun_fd, int epoll_fd, char *ip, char *proto, char *data, size_t data_size, BlockList const &block, struct stats &stats,
    ...
    if (data_size > 0 && !is_out_of_order) {
      ...
      debug("TCP %i: DATA: Sequence: %u (New Expected: %u) Size: %zu", fd, tcp_seq, them_seq, data_size);
    ...
    if (tcp_hdr->fin) {
      them_seq += 1;
      shutdown(fd, SHUT_WR);
      debug("TCP %i: Client has shutdown write half of stream %u %u %i", fd, us_seq, them_ack, close_rd);
      close_wr = true;
    ...
    if (tcp_hdr->ack && close_rd && !ack_rd && them_ack >= us_seq) {
      debug("TCP %i: Stream has acknowledged FIN %u %u %u with %zu remaining", fd, tcp_seq, them_ack, us_seq, unacknowledged.size());
      ack_rd = true;
    }
    ...
    if (unacknowledged.size()) {
      ...
      for (auto segment : unacknowledged) {
        if (segment.should_retransmit(cur_time)) {
          retransmit_packet(tun_fd, segment.data, segment.seq_start, segment.seq_len, false, true, close_rd);
          done++;
        }
      }
  ...
  bool on_data(int tun_fd, int epoll_fd, char *data, size_t data_size, struct stats &stats, timespec const &cur_time) {
